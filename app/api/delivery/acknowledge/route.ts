...
async function generatePODPdf(params: {
  ...
  try {
    ...
    console.warn('[POD] signature embed failed:', sigErr);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: urlData } = await supabase.storage
      .from(BUCKET)
      .createSignedUrl(pdfKey, 60 * 60 * 24 * 365 * 10);
    ...
    await addSOComment(
      so.soId,
      `Delivery acknowledged by ${name} on ${confirmedDateIST} IST.\nRef: ${referenceId}\nProof of Delivery PDF: ${pdfUrl}`
    );

    try {
      await updateSODeliveryStatus(so.soId, 'Confirmed');
    } catch (cfErr) {
      console.warn('[acknowledge] Custom field update failed:', cfErr);
    }

    return NextResponse.json({ success: true, pdfKey, pdfUrl, referenceId });
  } catch (err) {
